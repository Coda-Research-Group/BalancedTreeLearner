#!/bin/bash
#PBS -N batl_laion_clip768_100m_b300
# Merge the four independently trained LAION-2B CLIP768v2 100M trees, then run
# the versioned beam-300 sweep. Submit only after all four tree jobs succeed.
#PBS -l select=1:ncpus=16:ngpus=1:gpu_mem=16gb:gpu_cap=sm_89:cluster=fobos:mem=512gb:scratch_ssd=400gb
#PBS -l walltime=24:00:00
#PBS -j oe
#PBS -o /auto/brno2/home/jozefsprlak/logs/batl/laion_clip768_100m_beam300_search.log
#PBS -m abe
#PBS -M 536343@mail.muni.cz

set -euo pipefail

RESULT_NAME="laion_clip768v2_100m_gpu_t4_beam300_search"
...
BUILD_CONFIG_RELATIVE="experiments/configs/laion5b/laion5b_100m_clip768v2_lmi.yaml"
SEARCH_CONFIG_RELATIVE="experiments/configs/laion5b/laion5b_100m_clip768v2_lmi_beam300.yaml"
JOB_RELATIVE="experiments/scripts/laion5b/metacentrum_laion_clip768v2_100m_gpu_beam300_search.sh"
...
cd "$SCRATCHDIR/BATL"
BUILD_CONFIG_PATH="$SCRATCHDIR/BATL/$BUILD_CONFIG_RELATIVE"
CONFIG_PATH="$SCRATCHDIR/BATL/$SEARCH_CONFIG_RELATIVE"
MERGED_INDEX="$RESULT_DIR/index_confidence.pkl"
if ! cmp -s "$REFERENCE_CONFIG" "$BUILD_CONFIG_PATH"; then
    echo "Saved tree config differs from the current versioned build config: $BUILD_CONFIG_PATH" >&2
    exit 2
fi
...
search_config_path = Path(sys.argv[2])
build_payload = yaml.safe_load(build_config_path.read_text(encoding="utf-8"))
search_payload = yaml.safe_load(search_config_path.read_text(encoding="utf-8"))
for payload, label in ((build_payload, "build"), (search_payload, "search")):
    if not isinstance(payload, dict) or not isinstance(payload.get("evaluation"), dict):
        raise SystemExit(f"{label} config has no evaluation mapping.")

normalized_build = copy.deepcopy(build_payload)
normalized_search = copy.deepcopy(search_payload)
for key in ("beam_size", "num_leaves"):
    normalized_build["evaluation"].pop(key, None)
    normalized_search["evaluation"].pop(key, None)
if normalized_build != normalized_search:
    raise SystemExit(
        "Only beam_size and num_leaves may differ between build and search configs."
    )

cfg = load_experiment_config(search_config_path)
if cfg.beam_size != 300:
    raise SystemExit(f"Expected beam_size=300, got {cfg.beam_size}.")
if cfg.num_leaves != [100, 150, 200, 250, 300]:
    raise SystemExit(f"Unexpected num_leaves sweep: {cfg.num_leaves}.")
...
if tuple(database.shape) != expected_shape or database.dtype != np.dtype(np.float32):
    raise SystemExit(
        f"Unexpected database layout: shape={database.shape}, dtype={database.dtype}."
    )
with h5py.File(Path(cfg.dataset_ground_truth_path or ""), "r") as handle:
    if "knns" not in handle:
        raise SystemExit("Ground truth is missing the required 'knns' dataset.")
    ids = np.asarray(handle["knns"])
    if ids.ndim != 2 or cfg.num_queries not in ids.shape:
        raise SystemExit(f"Unexpected ground-truth shape: {ids.shape}.")
    if int(ids.min()) < 1 or int(ids.max()) > cfg.subset_size:
        raise SystemExit("Ground-truth IDs fall outside the configured database.")
build_times_path = Path(sys.argv[3])
if cfg.model.num_trees != 4:
    raise SystemExit(f"Expected num_trees=4, got {cfg.model.num_trees}.")
build_rows = []
for tree_index, path in enumerate(sys.argv[4:]):
...
if [ "$RUN_STATUS" -eq 0 ]; then
    ...
    set +e
    # 25 <= floor(65535 / (8 attention heads * beam 300)) = 27.
...
cp "$BUILD_CONFIG_PATH" "$OUTPUT_DIR/tree_build_config.yaml"
